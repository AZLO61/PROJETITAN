...
export function useBoardGeneratorController() {
  ...
  }, [mancheNumber, titanState.ordreJeu]);
  ...
  useEffect(() => {
    ...
    for (const t of titanState.players) {
      const colors = new Set(t.repaire.filter((c) => c !== "vert"));
      if (["bleu", "rose", "orange", "rouge"].every((c) => colors.has(c))) {
        setRainbowWinnerId(t.id);
        setActionLog((prev) => [...prev, `🌈 Arc-en-ciel ! Titan ${t.id} est le premier à posséder les 4 couleurs → +5 pts fin de partie.`]);
        break;
      }
    }
  ...
  const [teaAdrenaline, setTeaAdrenaline] = useState(false);
  const [tcAdrenaline, setTcAdrenaline] = useState(false); // Tout Casser +1 puissance
  ...
  const [bbAdrenaline, setBbAdrenaline] = useState(false);
  ...
  const [progCountdown, setProgCountdown] = useState(null);   // null | 1-5
  ...
  const [moveAdrenaline, setMoveAdrenaline] = useState(false);
  ...
  const captureSnapshot = useCallback(() => {
    setUndoStack((prev) => [...prev, {
      ...
      titanState: JSON.parse(JSON.stringify(titanState)),
      looseBlocks: JSON.parse(JSON.stringify(looseBlocks)),
      activePlayerId,
      phase,
  ...
  const handleUndo = useCallback(() => {
    setUndoStack((prev) => {
      ...
      // Appliquer le snapshot via setTimeout pour sortir du setter fonctionnel
      // et garantir que React re-render correctement chaque état
      setTimeout(() => {
        setState(JSON.parse(JSON.stringify(snap.state)));
        setTitanState(JSON.parse(JSON.stringify(snap.titanState)));
        setLooseBlocks(JSON.parse(JSON.stringify(snap.looseBlocks)));
        setActivePlayerId(snap.activePlayerId);
        setPhase(snap.phase);
        setPassifUsed(JSON.parse(JSON.stringify(snap.passifUsed)));
  ...
  // ── AUTO-RÉSOLUTION DIL/RAGE IA vs IA ──
  // Résout immédiatement les décisions où attaquant ET défenseur sont IA,
  // sans passer par la queue UI. Retourne les décisions nécessitant un humain.
  const autoResolveIaDecisions = useCallback((rawDecisions, curTitanModes, curPlayers) => {
    ...
    for (const d of rawDecisions) {
      ...
      // Les deux sont IA → résolution automatique
      const defender = curPlayers.find((t) => t.id === d.defenderId);
      const attacker = curPlayers.find((t) => t.id === d.attackerId);
      if (!defender || !attacker) continue;
      // ── Valeur marginale d'un bloc pour un Titan (heuristique scoring) ──
      // Retourne le gain de points apporté par UN bloc supplémentaire de cette couleur.
      // Orange : scoring par paires → un orange impair vaut 0, un pair complète une paire.
      // Rose : +10 bonus au plus grand nombre → on l'estime par un bonus flat si compétitif.
      function marginalValue(color, currentRepaire, allPlayers, selfId) {
        ...
        // Rose : ajouter une estimation du bonus +10 si on devient ou reste leader
        if (color === "rose") {
      ...
      if (d.type === "RAGE") {
        if (defender.repaire.length >= 2) {
          ...
          setActionLog((prev) => [...prev, `RAGE IA auto : T${d.attackerId} prend ${taken} (+${bestScore}pts) à T${d.defenderId}.`]);
        } else if (defender.adrenaline >= 1) {
          defender.adrenaline -= 1;
          setActionLog((prev) => [...prev, `RAGE IA auto (FAQ#5) : T${d.attackerId} prend 1 Adrénaline à T${d.defenderId}.`]);
        }
      } else if (d.type === "DIL") {
        ...
        const ranked = rep
          .map((color, idx) => ({
            color, idx,
            atkVal: marginalValue(color, attacker.repaire, curPlayers, d.attackerId),
          }))
        ...
        if (offered.length === 1) {
          ...
          setActionLog((prev) => [...prev, `DIL IA auto : T${d.defenderId} perd ${offered[0].color} (seul choix).`]);
        } else {
          const defValued = offered.map((o) => ({
            ...o,
            defVal: marginalValue(o.color, rep, curPlayers, d.defenderId),
          }));
          const defChoice = defValued.reduce((best, curr) => curr.defVal < best.defVal ? curr : best);
          const loseIdx = rep.indexOf(defChoice.color);
          if (loseIdx !== -1) rep.splice(loseIdx, 1);
          setActionLog((prev) => [...prev, `DIL IA auto : T${d.defenderId} perd ${defChoice.color} (valeur marginale ${defChoice.defVal}).`]);
        }
      }
    }
    return needHuman;
  ...
  const toggleProgCard = useCallback((cardId) => {
    setProgSelection((prev) => {
      ...
      if (next.length === 3) {
        // Démarre le compte à rebours 5s avant validation
        let countdown = 5;
  ...
  const teaMaxRange = PORTEE_TETE_EN_AVANT + (teaAdrenaline ? 1 : 0);
  ...
  const jouerTeteEnAvant = useCallback((targetKey) => {
    ...
    const actuallyUseAdrenaline = teaAdrenaline && (attacker.adrenaline || 0) >= 1;
  ...
  const bbMaxRange = PORTEE_BOING_BOING + (bbAdrenaline ? 1 : 0);
  ...
  const toggleBbMode = useCallback(() => { setBbMode((m) => !m); setBbDest(null); }, []);
  ...
  const jouerBoingBoing = useCallback(() => {
    ...
    captureSnapshot();
    const attacker = titanState.players.find((t) => t.id === selectedTitanId);
    const actuallyUseAdrenaline = bbAdrenaline && (attacker.adrenaline || 0) >= 1;
  ...
  const moveMaxRange = 2 + (moveAdrenaline ? 1 : 0);
  ...
  const toggleJnpMode = useCallback(() => { setJnpMode((m) => !m); setJnpSelected([]); }, []);
  ...
  const endGameReasons = checkEndGameTriggers(state.board, looseBlocks, apocalypseThreshold);

  const boardSignature3D = JSON.stringify({
  ...
  const stats = { bleu: 0, rose: 0, orange: 0, rouge: 0, vert: 0 };
  let occupiedCount = 0;
  Object.values(state.board).forEach((b) => {
    b.blocks.forEach((c) => (stats[c] = (stats[c] || 0) + 1));
    if (b.blocks.length > 0) occupiedCount++;
  });
  ...
  if (!setupDone) {
    return (
      <div style={{
        ...
        <h2 style={{ fontFamily: "'Bowlby One', sans-serif", color: "#FFD93D", fontSize: "1.3rem", marginBottom: 6 }}>
          BIG CITY — Configuration
        </h2>
        <p style={{ fontSize: ".82rem", color: "rgba(255,255,255,.6)", marginBottom: 20 }}>
          Paramètre la partie avant de lancer. Ces réglages sont <strong style={{ color: "#FFD93D" }}>verrouillés au démarrage</strong>.
        </p>

        <div style={{ marginBottom: 20 }}>
          <div style={{ color: "#FFD93D", fontFamily: "'Bowlby One', sans-serif", fontSize: ".85rem", marginBottom: 8 }}>
            Nombre de Titans
          </div>
          <div style={{ display: "flex", gap: 10 }}>
            {[3, 4].map((n) => (
              <button key={n} onClick={() => setNbJoueurs(n)} style={{
                flex: 1,
                background: nbJoueurs === n ? "linear-gradient(135deg,#ff9239,#FF2E63)" : "rgba(255,255,255,.08)",
                border: "1px solid rgba(255,255,255,.2)", borderRadius: 10, color: "#fff",
                padding: "12px 0", fontFamily: "'Bowlby One', sans-serif", fontSize: ".85rem", cursor: "pointer",
              }}>
                {n} Titans {n === 4 ? "(4 Manches)" : "(6 Manches)"}
              </button>
            ))}
          </div>
        </div>
              ...
              return (
                <div key={id} style={{
                  ...
                  <input
                    ...
                    maxLength={18}
                    style={{
                      flex: 1, minWidth: 100, background: "rgba(255,255,255,.08)",
                      border: `1px solid ${tc?.accent || "rgba(255,255,255,.2)"}55`, borderRadius: 8,
                      color: tc?.accent, padding: "7px 10px", fontSize: ".82rem",
                  ...
                  <div style={{ display: "flex", gap: 6 }}>
                    {["humain", "ia"].map((m) => (
                      <button key={m} onClick={() => setTitanModes((prev) => ({ ...prev, [id]: m }))} style={{
                        ...
                        padding: "5px 12px", fontSize: ".75rem", fontWeight: 700, cursor: "pointer",
                      }}>
                        {m === "humain" ? "👤 Humain" : "🤖 IA"}
                      </button>
                    ))}
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <label style={{
            ...
            borderRadius: 10, padding: "12px 14px", cursor: "pointer", fontSize: ".82rem",
          }}>
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <div style={{
            ...
            />
            <span style={{ fontSize: ".76rem", color: "rgba(255,255,255,.55)" }}>
        ...
        <button
          ...
          style={{
            ...
            borderRadius: 10, color: "#0E0420", fontWeight: 700, padding: "14px 0",
            fontFamily: "'Bowlby One', sans-serif", fontSize: ".9rem", cursor: "pointer",
          }}
        >
          🏙️ Lancer la partie
        </button>
